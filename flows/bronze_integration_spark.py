...
def upload_csv_to_sources(file_path: str, object_name: str) -> str:
    ...
    logger.info(f"✅ Uploaded {object_name} to {BUCKET_SOURCES}")
    return object_name
...
def bronze_ingestion_flow() -> Dict[str, str]:
    ...
    # Upload vers sources
    clients_name = upload_csv_to_sources(str(clients_file), "clients.csv")
    purchases_name = upload_csv_to_sources(str(purchases_file), "purchases.csv")

    # Copy vers bronze avec structure de dossiers
